...
let dim = parseFloat(d3.select(".game-grid").style("width"));
...
for (i = 0; i < size; i++) {
  ...
  for (j = 0; j < size; j++) {
    ...
    let spotObj = {
      col_spotter_id: "colSpotter_" + k,
      row_spotter_id: "rowSpotter_" + k,
      showRow: false,
      showCol: false
    };
    ...
    gameState.push({
      ...
      y: i * sp,
      spotters: spotObj,
      target: colSolutionObject,
      current: { r: null, g: null, b: null },
      occ_Tile_H: null,
      occ_Tile_V: null
    });
...
const tiles = [
  ...
  }, //col2
  {
    ...
    location: "base",
    orient: "vertical",
    colours: [null, null, { r: 0, g: 255, b: 0 }]
  }, //col3
  {
    tile_id: "tile_6",
    location: "base",
  ...
  }, //col3
  {
    ...
    location: "base",
    orient: "vertical",
    colours: [{ r: 0, g: 0, b: 255 }, null, { r: 255, g: 0, b: 0 }]
  }, //col4
  {
    tile_id: "tile_8",
    location: "base",
  ...
  }, //col6
  {
    ...
    location: "base",
    orient: "vertical",
    colours: [{ r: 255, g: 0, b: 0 }, null, { r: 0, g: 255, b: 0 }]
  }, //col6
  ...
  //horiz cards
  {
    ...
    location: "base",
    orient: "horizontal",
    colours: [{ r: 0, g: 0, b: 255 }, null, { r: 0, g: 255, b: 0 }]
  }, //row1
  {
    tile_id: "tile_14",
    location: "base",
    orient: "horizontal",
    colours: [null, { r: 255, g: 0, b: 0 }, { r: 0, g: 0, b: 255 }]
  }, //row1
  {
    tile_id: "tile_15",
    location: "base",
  ...
  }, //row2
  {
    ...
    location: "base",
    orient: "horizontal",
    colours: [{ r: 0, g: 255, b: 0 }, null, { r: 0, g: 0, b: 255 }]
  }, //row2
  {
    tile_id: "tile_17",
    location: "base",
    orient: "horizontal",
    colours: [null, { r: 0, g: 0, b: 255 }, { r: 0, g: 0, b: 255 }]
  }, //row3
  {
    tile_id: "tile_18",
    location: "base",
    orient: "horizontal",
    colours: [null, null, { r: 255, g: 0, b: 0 }]
  }, //row3
  {
    tile_id: "tile_19",
    location: "base",
  ...
  }, //row4
  {
    ...
    location: "base",
    orient: "horizontal",
    colours: [{ r: 0, g: 255, b: 0 }, null, null]
  }, //row4
  {
    tile_id: "tile_21",
    location: "base",
  ...
  }, //row6
  {
    ...
    location: "base",
    orient: "horizontal",
    colours: [{ r: 255, g: 0, b: 0 }, { r: 0, g: 255, b: 0 }, null]
  } //row6
];
...
/*  offsetX = d3.event.x - 6
  ...
  */
  ...
  let hSVG = hTile.append("svg").attr("width", "100%").attr("height", "100%");
...
const dragHandlerVertical = d3
  ...
  .on("drag", function () {
    ...
    d3.select(movingId).style("left", x).style("top", y);
  ...
  .on("end", function () {
    ...
    //  console.log("outside");
      ...
      for (i = 0; i < colObj.length; i++) {
        let tempObj = gameState.find(
          (o) => o.box_id === "box_" + (i * size + boxNum)
        );
      ...
      if (viablePosition) {
        ...
        for (j = 0; j < colObj.length; j++) {
          ...
          if (thisRGB == null) {
            ...
            gameState[posIndex].current.r = d3.min([
              thisRGB.r + gameState[posIndex].current.r,
              255
            ]);
            gameState[posIndex].current.g = d3.min([
              thisRGB.g + gameState[posIndex].current.g,
              255
            ]);
            gameState[posIndex].current.b = d3.min([
              thisRGB.b + gameState[posIndex].current.b,
              255
            ]);
          }
        ...
        tmpDiv
          ...
          .ease(d3.easeBounce)
          .style("left", () => {
            return gameState[posIndex].x + ggLeft + sp * 0.15 + "px";
          })
          .style("top", () => {
            return gameState[posIndex].y + ggTop + sp * 0.15 + "px";
          })
          .on("end", () => {
            tmpDiv.remove();
          });
...
const dragHandlerHorizontal = d3
  .drag()
  .on("start", function () {
    div = d3.select(this);
...
const dragFromGridHorizontal = d3
  .drag()
  .on("start", function () {
    //add a new div for drag
    //remove colour data from gameState
...
let dragMethod = dontDrag(svgGame);
////////////     HELPER FUNCTIONS      ///////////
...
function showDivs(event, divName) {
  ...
  d3.select(divName).style("display", "flex");
  ...
  if (divName === "#rowsContainer") {
    ...
    dragMethod = dragFromGridHorizontal(svgGame);
    updateRowSpotters();

    //enable dragFromGridHorizontal
  } else {
    //the opposite.
    dragMethod = dragFromGridVertical(svgGame);
    updateColSpotters();
  }
}

function updateColSpotters() {
  d3.selectAll(".col-spotter").attr("class", "col-spotter inactive-spotter");

  d3.selectAll(".row-spotter").attr("class", "row-spotter inactive-spotter");

  for (i = 0; i < gameState.length; i++) {
    if (gameState[i].spotters.showCol === true) {
      let spotId = "#" + gameState[i].spotters.col_spotter_id;
      d3.select(spotId).attr("class", "col-spotter active-spotter");
    }
  }
}
...
function updateRowSpotters() {
  ...
  for (i = 0; i < gameState.length; i++) {
    if (gameState[i].spotters.showRow === true) {
      ...
      d3.select(spotId).attr("class", "row-spotter active-spotter");
    }
  }
}
...
function findBox(x, y, location_array, boxSize) {
  ...
  for (i = 0; i < location_array.length; i++) {
    if (
      x > location_array[i].x + boxSize &&
      x < location_array[i].x + boxSize * 2
    ) {
      //great, correct column
      if (
        y > location_array[i].y - boxSize / 2 + 5 &&
        y < location_array[i].y + boxSize
      ) {
        id = location_array[i].box_id;
      }
    }
